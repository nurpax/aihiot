...
let acc_adler (adler1,adler2) v =
  (I32.rem (I32.add adler1 (I32.of_int (v land 255))) (I32.of_int 65521),
   I32.rem (I32.add adler1 adler2) (I32.of_int 65521))
...
let write_png_chnl chnl pixels w h =
  ...
  let output_adler_byte (a1,a2) v =
    let b = v land 255 in
    output_byte chnl b;
    acc_adler (a1,a2) b in
  ...
  let rec write_rows ((crc,adler1,adler2) as acc) y =
    if y < h then
      ...
      let adler = 
        output_adler_byte (adler1,adler2) 0 in
      let rec output_row x ((a1,a2) as acc) =
        if x < w then
          ...
          acc in
      let (a1,a2) = (output_row 0 adler) in
      write_rows (crc,a1,a2) (y+1)
    else 
      acc in
  
  let (crc,adler1,adler2) = 
    write_rows ((I32.of_string "0x13e5812d"), I32.one, I32.zero) 0 in
  let bytes = [adler2 >> 8; adler2; adler1 >> 8; adler1] in
  let crc = List.fold_left (fun a v -> output_crc_byte a (I32.to_int v)) crc bytes in
  write_dword chnl (I32.lognot crc);
